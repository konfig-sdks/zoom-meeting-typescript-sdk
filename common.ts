...
import { readableStreamToString, ZoomMeetingError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ZoomMeetingError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ZoomMeetingError(e, e.response?.data)
                }
                if (innerError instanceof ZoomMeetingError) {
                    // Got "ZoomMeetingError" from the above try block
                    throw innerError;
                }
